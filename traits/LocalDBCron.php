...
trait LocalDBCron {
	...
	public function every_ten_minute_cron() {
		...
		if ( $dir_files && is_array( $dir_files ) && count( $dir_files ) >= 1 ) {
			...
			foreach ( $dir_files as $single_file ) {
				if ( isset( $single_file['lastmodunix'] ) && $single_file['lastmodunix'] && isset( $single_file['type'] ) && $single_file['type'] == 'f' && isset( $single_file['name'] ) && ( $single_file['name'] == 'labgrown.csv' || $single_file['name'] == 'natural_diamonds.csv' ) ) {
	...
	public function get_diamonds_from_csv() {
		...
		if ( $dir_files && is_array( $dir_files ) && count( $dir_files ) >= 1 ) {
			...
			foreach ( $dir_files as $single_file ) {
				if ( isset( $single_file['type'] ) && $single_file['type'] == 'f' && isset( $single_file['name'] ) && ( $single_file['name'] == 'labgrown.csv' || $single_file['name'] == 'natural_diamonds.csv' ) ) {
	...
	public function add_file_to_import_que( $files_list ) {
		...
		$listWorksheetInfo = $this->list_worksheet_info( $first_file['absolute_path'] );
