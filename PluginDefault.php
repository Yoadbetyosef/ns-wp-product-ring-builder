...
class PluginDefault extends Plugin{
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			add_filter(
				'plugin_action_links_' . plugin_basename( OTW_WOO_RING_BUILDER_PLUGIN_FILE ),
				array( $this, 'plugin_action_links' )
			);

			register_activation_hook(
				plugin_basename( OTW_WOO_RING_BUILDER_PLUGIN_FILE ),
	...
	public function wp_all_export_available_data( $available_data ) {
		if ( isset( $available_data['existing_meta_keys'] ) &&
			isset( $available_data['woo_data'] )
		) {
			$new_keys = array(
				'otw_woo_variation_3d_model',
				'otw_woo_variation_gallery_images',
				'otw_woo_variation_video_url',
			);

			$available_data['existing_meta_keys'] = array_merge(
				$new_keys,
				$available_data['existing_meta_keys']
			);
		}
