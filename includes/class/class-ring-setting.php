...
class Ring_Builder_Setting {
	...
	public function __construct() {
		...
		add_action( 'admin_menu', array( $this, 'gcpb_setting_page' ), 11 );

		add_action( 'admin_notices', array( $this, 'gcpb_admin_notice' ) );

		add_action( 'admin_enqueue_scripts', array( $this, 'gcpb_admin_script' ) );

		add_action( 'admin_post_gcpb_process_setting', array( $this, 'gcpb_setting_form_action' ) );
	}

	public function gcpb_admin_script() {
		$screen = get_current_screen();

		if ( $screen->id !== 'toplevel_page_gcpb-setting' ) {
			return;
		}

		wp_enqueue_style( 'gcpb-admin', plugins_url( 'assets/admin/css/gcpb-admin.css', dirname( __DIR__ ) ), array(), '1.1.2' );

		wp_enqueue_script( 'jquery-ui-draggable' );

		wp_enqueue_script( 'gcpb-admin', plugins_url( 'assets/admin/js/gcpb-admin.js', dirname( __DIR__ ) ), array( 'jquery' ), '2.1.0', true );
			...
			case 'repeater':
				...
				if ( ! empty( $attributes ) ) {
					foreach ( $attributes as $attribute ) {
						...
						$sub_terms[ $attribute->attribute_name ]['sub_terms'] = get_terms(
							array(
