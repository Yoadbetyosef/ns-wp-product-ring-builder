...
class Ring_Builder {
	...
	public function __construct() {
		...
		add_action( 'wp_enqueue_scripts', array( $this, 'ring_builder_frontend_script' ) );

		add_action( 'wp_ajax_nopriv_fetch_products', array( $this, 'fetch_products_func' ) );

		add_action( 'wp_ajax_fetch_products', array( $this, 'fetch_products_func' ) );

		add_action( 'wp_ajax_nopriv_fetch_product_single_variation', array( $this, 'fetch_product_single_variation_func' ) );

		add_action( 'wp_ajax_fetch_product_single_variation', array( $this, 'fetch_product_single_variation_func' ) );

		add_action( 'gcpb_product_loop_attributes', array( $this, 'gcpb_product_loop_attributes_func' ), 10, 2 );

		add_action( 'gcpb_single_product_attributes', array( $this, 'gcpb_single_product_attributes_func' ), 10, 3 );

		add_action( 'wp_ajax_nopriv_gcpb_single_product_variation_data', array( $this, 'gcpb_single_product_variation_data_func' ) );

		add_action( 'wp_ajax_gcpb_single_product_variation_data', array( $this, 'gcpb_single_product_variation_data_func' ) );

		add_action( 'wp_ajax_nopriv_gcpb_link_single_product', array( $this, 'gcpb_link_single_product_func' ) );

		add_action( 'wp_ajax_gcpb_link_single_product', array( $this, 'gcpb_link_single_product_func' ) );

		add_action( 'wp_ajax_nopriv_gcpb_fetch_3d_model_image', array( $this, 'gcpb_fetch_3d_model_image_func' ) );

		add_action( 'wp_ajax_gcpb_fetch_3d_model_image', array( $this, 'gcpb_fetch_3d_model_image_func' ) );
	}

	public function init() {
		add_action( 'product_steps', array( $this, 'product_steps_func' ) );

		add_action( 'product_filters', array( $this, 'product_filters_func' ) );

		add_action( 'prb_product_listing', array( $this, 'prb_product_listing_func' ), 10, 1 );
	}
	...
	public function ring_builder_frontend_script() {
		...
		wp_register_script( 'gcpb-builder', plugins_url( 'assets/js/ring-builder.js', dirname( __DIR__ ) ), array( 'jquery' ), get_file_time( $script_abs_path ), true );

		wp_enqueue_script( 'gcpb-builder' );

		$ajax_variables = array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) );
		...
		wp_register_style( 'gcpb-builder', plugins_url( 'assets/css/ring-builder.css', dirname( __DIR__ ) ), array(), get_file_time( $script_abs_path ) . rand() );
						...
						if ( isset( $attributes[ 'pa_' . $key ] ) && ! empty( $attributes[ 'pa_' . $key ] ) ) {
							...
							<div class="gcpb-available-container">
								<ul class="gcpb-available-wrapper gcpb-scrollbar gcpb-horizontal js-gcpb-loop-attribute" data-attribute="<?php echo $key; ?>">
									<?php
									foreach ( $attribute_item->get_terms() as $metal_attribute ) {
						...
						if ( isset( $attributes[ 'pa_' . $key ] ) && ! empty( $attributes[ 'pa_' . $key ] ) ) {
							...
							<div class="gcpb-available-container">
								<ul class="gcpb-available-wrapper gcpb-scrollbar gcpb-horizontal js-gcpb-single-variation" data-attribute="<?php echo $key; ?>">
									<?php
									foreach ( $attribute_item->get_terms() as $attribute_item ) {
